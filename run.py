...
from Tool import Zip, Path, Console, sleep, strftime, localtime, ConfigParser
...
try:
  ...
  def Sub(PROJECT):
    ...
    if Except(select).Select()[1] == True:
      if select == '0':
        ...
        subprocess.run([str(Plug) + ' ' + str(PROJECT)], shell=True), input()
      else:
        console.print('\n 没有此选项\n', style='bold red'), sleep(0.5)
  ...
  if Path('Tool').is_dir():
    ...
    if platform.machine() == 'x86_64':
        down = cfp.get('x86_64','down')
        if not Path(down).is_dir():
          Path(down).mkdir(mode=0o777, exist_ok=True)
        
    elif platform.machine() == 'aarch64' or platform.machine() == 'armv8l' :
        download = Directory_Path(Path('/'), cfp.get('aarch64','Down'))
        down = download.str()
        if not Path(down).is_dir():
          Path(cfp.get('x86_64','Down')).mkdir(mode=0o777, exist_ok=True)
          down = cfp.get('x86_64','Down')
